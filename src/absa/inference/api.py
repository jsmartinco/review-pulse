...
def normalise_aspects(aspects: list[str]) -> list[str]:
    ...
    if not result: raise ValueError("Provide at least one non-empty aspect")
    return result


def predict_aspects(review: str, aspects: list[str], model_name: str, predictor) -> list[dict]:
    if not review.strip(): raise ValueError("Review must not be empty")
    return [predictor.predict(review.strip(), aspect, model_name) for aspect in normalise_aspects(aspects)]
