...
def train_distilbert(train_rows, test_rows, *, epochs: int = 2, batch_size: int = 8):
    splits = split_official_data(train_rows, test_rows)
    tokenizer = AutoTokenizer.from_pretrained("distilbert-base-uncased")
    model = ABSADistilBERT.from_pretrained_absa()
    ...
    for _ in range(epochs):
        for batch in loader:
            ...
            optimiser.zero_grad(); loss=torch.nn.CrossEntropyLoss()(model(**batch).logits, labels); loss.backward(); optimiser.step()
    ...
    def score(rows):
        ...
        with torch.no_grad():
            for batch in loader:
                batch.pop("labels")
                predicted.extend(model(**batch).logits.argmax(1).tolist())
