...
The optional review-only TextCNN is a low-compute, non-recurrent sentence-level baseline rather than a reproduction of the cited Twitter CNN. A bounded three-configuration gate selects filter widths/counts by development macro-F1 without evaluating the official test. Its artifact persists the selected widths, filter count, global-max pooling, short-input padding and full #91 run record. The predictor repeats review-level outputs per supplied aspect and explicitly marks token evidence as unsupported. Its protocol is documented in `docs/dle602-a3/text-cnn.md`; shared six-model results remain gated under #96.
...
- the current full-suite result on the #94 candidate branch is **250 passed, 8 skipped**.
